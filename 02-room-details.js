...
function getRoomByDinosaurName(dinosaurs, rooms, dinosaurName) {
  let foundDinoId = null;
  let foundRoomName = null;

  for (let i = 0; i < dinosaurs.length; i++) {
    const element = dinosaurs[i];
    if (element.name === dinosaurName) {
      foundDinoId = element.dinosaurId;
      console.log (element)
    }
  }

  if (!foundDinoId) {
    return "Dinosaur with name '" + dinosaurName + "' cannnot be found.";
  }
  for (let j = 0; j < rooms.length; j++) {
    const element = rooms[j];

    if (element.dinosaurs.includes(foundDinoId)) {
      foundRoomName = element.name;
    }
  }
  if (!foundRoomName) {
